...
def signup(request):
    if request.method == 'POST' and 'btnsingnup' in request.POST:
        fname = None
        lname = None
        address = None
        address2 = None
        ...
        if fname and lname and address and address2 and city and state and zip_number and email and username and password:
            if terms == 'on':
                ...
                if User.objects.filter(username=username).exists():
                    messages.error(request, 'this username is exist')
                else:
                    #check if email is taken
                    if User.objects.filter(email=email).exists():
                        messages.error(request, 'this email is exist')  
                    else:
                        patt = "^[\w]+[\w.+-]*@[\w-]{2,}(\.[\w-]{2,})?\.[a-zA-Z]{2,}$"
    ...
    if request.method == 'POST' and 'btnsave' in request.POST:
        ...
        if request.user is not None and request.user.id != None:
            ...
            if request.POST['fname'] and request.POST['lname'] and request.POST['address'] and request.POST['address2'] and request.POST['city'] and request.POST['state'] and request.POST['zip'] and request.POST['email'] and request.POST['user'] and request.POST['pass']:
                ...
                userprofile.adderss = request.POST['address']
                userprofile.adderss2 = request.POST['address2']
                ...
                messages.success(request, 'you data has been saved') 
...
def product_favorites(requst, pro_id):

    if requst.user.is_authenticated and not requst.user.is_anonymous:
        pro_fav = Product.objects.get(pk=pro_id)
        if UserProfile.objects.filter(user=requst.user, product_favorites =pro_fav).exists():
            messages.success(requst, 'Already product in the favorit')

        else:
            userprofile = UserProfile.objects.get(user=requst.user)
            userprofile.product_favorites.add(pro_fav)
            messages.success(requst, 'Product Has Been Favorited')

    else:
        messages.error(requst, 'You Must Be Logged in')
